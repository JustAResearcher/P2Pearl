...
def test_v5_target_limit_round_trip_and_commits():
    s = _sample(version=5, share_target=(1 << 240), target_limit=(1 << 244))
    raw = s.serialize()
    back = ShareBlock.deserialize(raw)
    assert back.share_target == (1 << 240)
    assert back.target_limit == (1 << 244)
    assert _sample(version=5, share_target=(1 << 240), target_limit=(1 << 243)).share_id() != s.share_id()
